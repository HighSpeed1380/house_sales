...
(function(window, klass, Util){
    ...
    PhotoSwipe.Image.ImageClass = klass({
        ...
        initialize: function(refObj, src, caption, metaData){
            ...
            if ($(this.refObj).attr('accesskey') == 'local') {
                this.imageEl = document.createElement('video');
                this.imageEl.width = 560;
                this.imageEl.height = 315;
                this.imageEl.controls = true;
                this.imageEl.accessKey = src;

                var source = document.createElement('source');
                source.type = 'video/mp4';
                this.imageEl.appendChild(source);
            } else if ($(this.refObj).attr('accesskey') == 'youtube') {
                this.imageEl = document.createElement('iframe');
                this.imageEl.width = 560;
                this.imageEl.height = 315;
                this.imageEl.accessKey = src;
                this.imageEl.src = '';
        ...
        load: function(){
            ...
            if (this.imageEl.tagName.toLowerCase() == 'img') {
...
(function(window, klass, Util){
    ...
    PhotoSwipe.Carousel.CarouselClass = klass({
        ...
        addCacheImageToItemEl: function(cacheImage, itemEl){
            ...
            //flynax
            var d = cacheImage.imageEl.nodeName.toLowerCase() == 'video' ? 'block' : 'none';
        ...
        videoHandler: function(){
            var index = $('.ps-carousel-content > div').length < 3 ? 0 : 1;
            var $video = $('.ps-carousel-content > div:eq(' + index + ') > *');
            var $source = $video.find('source');
            var $ps   = $('.ps-uilayer');

            var tagName = $video.prop('tagName').toLowerCase();

            if (tagName != 'img') {
                if (tagName == 'video') {
                    if (!$source.attr('src')) {
                        $source.attr('src', $video.attr('accesskey'));
                        $video.removeAttr('src');
                        $video.load();
                    }
                } else {
                    if (!$video.attr('src')) {
                        $video.attr('src', $video.attr('accesskey'));
                    }
                }

                var cont_height = $('.ps-carousel-content').height();
                var video_height = $video.height();
                var new_video_height = (cont_height - video_height) / 2;
                var top_position = $video.offset().top + video_height;

                $ps.css({
                    height: new_video_height + 'px',
                    top: top_position
                });
            } else {
                $ps.css({
